<?php
    ...
    {
        $election_id = mysql_escape_string($_POST['election_id']);
        $getTime = mysql_query("SELECT * FROM elections WHERE election_id='".$election_id."'");
        $results = mysql_fetch_array($getTime);
        if ($results['open'] == 'U')
        {
            $newTime = time() + $results['delete_time'];
            $openElection = mysql_query("UPDATE elections SET open='T', delete_time='".$newTime."' WHERE election_id='".$election_id."'");
        }
        else
            header("Location: reopen_election.php?id=".$election_id."");
    }
    
    if (isset($_POST['close']))
    {
        $election_id = mysql_escape_string($_POST['election_id']);
        $closeElection = mysql_query("UPDATE elections SET open='F', delete_time='' WHERE election_id='".$election_id."'");
    }
?>
...
<body>
    ...
    <div id="container">
        <center>
            ...
            <a href="create_election.php"><input type="button" value="Create Election" class="form_btn" style="display: block; margin-bottom: 20px; width: 180px;" /></a>
            <table style="width: 700px;" cellpadding="1" cellspacing="0" bgcolor="#F5F5F5" border="1" bordercolor="#E5E5E5">
                ...
                <?php
                    $getElections = mysql_query("SELECT * FROM elections WHERE open='T'");
