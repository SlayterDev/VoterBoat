<?php
    ...
    if (isset($_POST['submit']))
    {
        ...
        if (empty($user_name) || $user_name == 'Username')
            ...
            header("Location: add_admin.php?error=5");
        else if (strlen($password) < 5 || strlen($password) > 20)
            header("Location: add_admin.php?error=6");
        else
        {
            $checkUser = mysql_query("SELECT * FROM users WHERE user_name='".$user_name."'");
            if (mysql_num_rows($checkUser) == 0)
            {
                $addAdmin = mysql_query("INSERT INTO users (user_name, password, permissions) VALUES ('".$user_name."', '".$hashed_password."', 'Admin')");
                header("Location: add_admin.php?error=4");
            }
            else
            {
                header("Location: add_admin.php?error=3");
            }
        }
    }
